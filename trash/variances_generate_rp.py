...
list_params = [EfficientSU2(num_qubits=n, reps=6).num_parameters for n in list_qubits]
result = {"qubits": list_qubits}
ps = [1 / 4, 1 / 2, 3 / 4, 1]
result["Variances"] = np.zeros((len(ps), len(list_qubits)))

for i, n in tqdm(enumerate(list_qubits)):
    list_omega = [1 / list_params[i] ** p for p in ps]
    jobs = (delayed(qubit_variance)(n, w) for w in list_omega)
    variances = Parallel(n_jobs=15)(jobs)
    result["Variances"][:, i] = np.array(variances).reshape((len(list_omega),))
    np.save("variance/variance_data_rps.npy", result, allow_pickle=True)
