...
def reconstruct_cfield(K1,K2,cfield,x0):
    fun = lambda c: np.linalg.norm(K1@c + K2@(c+cfield))
...
def expected_state(hamiltonian:SparsePauliOp | tuple[np.ndarray, KLocalPauliBasis], beta: float):
...
def noise_hamiltonian(
    ...
    noiseloc_upper = KLocalPauliBasis(noise_locality, num_qubits).size
    noiseloc_lower = KLocalPauliBasis(noise_locality - 1, num_qubits).size
    H_vec = np.zeros(basis.size)
    H_vec[noiseloc_lower:noiseloc_upper] = np.random.normal(
        0, 1, size=noiseloc_upper - noiseloc_lower
    ) / (noiseloc_upper - noiseloc_lower)
    return basis.vector_to_pauli_op(H_vec) / np.linalg.norm(H_vec)


def error_bound(singular_values: np.ndarray):
    return np.sqrt(np.sum(1 / singular_values[:-1]))
...
def lattice_hamiltonian(
    ...
    H = SparsePauliOp(two_ops) * two_coeff + SparsePauliOp(one_ops) * one_coeff
    if vector:
        return KLocalPauliBasis(2,num_qubits,periodic).pauli_to_vector(H)
...
def classical_learn_hamiltonian(
    state: QuantumCircuit | DensityMatrix|Statevector, klocality: int, periodic: bool = False
) -> np.ndarray:
