...
variances = np.load("variance/variance_data_rs_omega.npy", allow_pickle=True).item()
...
for i, o in enumerate(omegas):
    axs.plot(
        ...
        markersize=2 * mksize,
        # color=colors[i],
        label=r"$r=$" + f"{o:.1e}" + r"$\frac{1}{\sqrt{m}}$",
    )
axs.set_xlabel(r"n")
...
plt.savefig("plots/variance_constdepth_rsqrt.png")
plt.show()
