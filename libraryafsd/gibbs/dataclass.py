...
def get_results(folder_path: str,learning_locality=None) -> list[GibbsResult]:
    """Loads all results in a folder"""
    gibbs_result_list = []
    for file in os.listdir(folder_path):
        if file.endswith(".npy"):
            path = os.path.join(folder_path, file)
            gibbs_result_list.append(GibbsResult.load(path,learning_locality))
...
class GibbsResult:
    ...
    def __post_init__(self):
        try:
            if self.cfaulties is None:
                ...
                cori = np.array(self.coriginal).copy()
                self.coriginal = np.zeros(self.cfaulties[0].size)
                self.coriginal[:cori.size] = np.array(cori)
        except:
            pass
    ...
    @classmethod
    def load(self, path,learning_locality:int=None):
        """
        ...
        if learning_locality is not None:
            dictionary["klocality"] = learning_locality
            dictionary["cfaulties"]=None
